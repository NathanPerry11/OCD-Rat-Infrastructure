...
Table~\ref{tab:req_design} maps key requirements to the corresponding design modules, explaining the rationale behind each decision.

\begin{table}[H]
\centering
\caption{Mapping Between System Requirements and Design Modules}
\label{tab:req_design}
\begin{tabularx}{\textwidth}{|p{3cm}|p{3cm}|X|}
\hline
\textbf{Requirement ID} & \textbf{Design Module} & \textbf{Design Decision / Rationale} \\ \hline
FUNC.R.1 -- Store and manage behavioral datasets & Data Schema and Storage Module (\mref{mDSS}) & Implemented a PostgreSQL schema supporting spatial-temporal data, video references, and metadata to ensure efficient storage and retrieval. \\ \hline
FUNC.R.2 -- Query and filter trials & API Layer Module (\mref{mAPI}) & REST API endpoints allow parameterized and natural language-based querying of trial data. \\ \hline
FUNC.R.3 -- Visualize trajectories and metrics & Data Visualization Module (\mref{mDV}) & React components and charting libraries render spatial trajectories, heatmaps, and statistical summaries. \\ \hline
APP.R.1 -- Enable intuitive researcher interaction & Front-End Interface Module (\mref{mUI}) & Accessible, responsive UI allows non-technical users to explore, filter, and download datasets. \\ \hline
APP.R.2 -- Natural language querying & NLP Query Processor Module (\mref{mNLP}) & Translates user queries in natural language into structured database queries. \\ \hline
SYS.R.1 -- Scalable and performant system & Data Processing Pipeline Module (\mref{mDPP}) & Parallelized pipelines and efficient indexing handle large datasets and multiple concurrent users. \\ \hline
SYS.R.2 -- Secure and reliable data access & Authentication and Access Control Module (\mref{mAAC}) & User authentication, role-based access, and HTTPS protocols ensure secure access to sensitive research data. \\ \hline
SYS.R.3 -- Extensible architecture for future analysis tools & Software Decision Modules (\mref{mSDM}) & Modular design with clear interfaces allows new algorithms and tools to be integrated without disrupting existing components. \\ \hline
\end{tabularx}
\end{table}
...
ROFT.R.2 & \mref{mFEM}, \mref{mSDM} \\
CAP.R.1  & \mref{mAPI}, \mref{mDSS} \\
CAP.R.2  & \mref{mHH}, \mref{mDSS} \\
SOE.R.1  & \mref{mAPI}, \mref{mSDM} \\
SOE.R.2  & \mref{mAPI}, \mref{mDPP}, \mref{mSDM} \\
LON.R.1  & \mref{mFEM}, \mref{mSDM} \\
LON.R.2  & \mref{mAPI} \\
EPE.R.1  & \mref{mHH}, \mref{mAPI} \\
EPE.R.2  & \mref{mHH}, \mref{mAPI} \\
EPE.R.3  & \mref{mHH} \\
WE.R.1   & \mref{mHH}, \mref{mUI}, \mref{mAPI} \\
IWAS.R.1 & \mref{mAPI}, \mref{mDSS}, \mref{mSDM} \\
PROD.R.1 & \mref{mSDM} \\
PROD.R.2 & \mref{mSDM} \\
REL.R.1  & \mref{mSDM} \\
REL.R.2  & \mref{mSDM} \\
MAI.R.1  & \mref{mFEM}, \mref{mSDM} \\
MAI.R.2  & \mref{mAPI}, \mref{mDSS} \\
SUP.R.1  & \mref{mUI} \\
SUP.R.2  & \mref{mUI} \\
ADA.R.1  & \mref{mHH}, \mref{mAPI} \\
ADA.R.2  & \mref{mHH}, \mref{mUI}, \mref{mAPI} \\
ADA.R.2  & \mref{mAPI}, \mref{mSDM} \\
INT.R.1  & \mref{mAAC}, \mref{mFEM}, \mref{mSDM} \\
IMM.R.1  & \mref{mAAC}, \mref{mSDM} \\
IMM.R.2  & \mref{mAPI}, \mref{mAAC}, \mref{mSDM} \\
LEG.R.1  & \mref{mSDM} \\
STA.R.1  & \mref{mSDM} \\
STA.R.2  & \mref{mSDM} \\
WR.1     & \mref{mDV}, \mref{mDPP} \\
WR.2     & \mref{mDSS}, \mref{mDPP} \\
WR.3     & \mref{mDPP}, \mref{mSDM} \\
...
\acref{acSecurityAndDependencyUpdates} & \mref{mSDM}\\
