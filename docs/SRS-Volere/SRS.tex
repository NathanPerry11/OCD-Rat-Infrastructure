...
\hline
BUC1: Get Result Set & Researcher & Query dataset & Filtered dataset returned & FUNC.R.1, FUNC.R.2, FUNC.R.3\\
\hline
BUC2: Download Data & Researcher & Download trials & Files downloaded  & FUNC.R.6 \\
\hline
BUC3: Get Data Visualizations & Researcher & Generate visualizations based on a requested data set & Graphical visualizations related to relevant dataset & FUNC.R.5\\
\hline
BUC4: Get Behavioural Metrics & Researcher & Generate statistical metrics of rat behaviour & Statistical breakdown of behavioural metrics in relevant dataset & FUNC.R.4\\
\hline
...
\textbf{FUNC.R.1:} & The program shall filter data to produce results based on predefined labels. & 1 & When a user selects a filter, the system provides a set that matches the labels to 100\% accuracy. & 1/10/2025 \\
\textbf{FUNC.R.2:} & The program shall generate results based on Natural Language Processing (NLP). & 1 & Given a user query, the system returns one dataset. & 1/10/2025 \\
\textbf{FUNC.R.3:} & The program shall provide categorized data allowing the user to browse different predefined sets. & 1 & The program should have at least 3 predetermined data sets for users to browse. & 1/10/2025 \\
\textbf{FUNC.R.4:} & The program shall provide behavioural categorization for each trial and behavioural metrics for result sets. & 1 & Each trial has at least one categorization. & 1/10/2025 \\
\textbf{FUNC.R.5:} & The program shall create visuals based on the results set. & 1 & For each result set, there is at least one corresponding visual. & 1/10/2025 \\
\textbf{FUNC.R.6:} & The program shall download data. & 1,2 & Users can export data sets or visualizations, verified against the first 100 rows. & 1/10/2025 \\
\textbf{FUNC.R.7:} & The program shall be easily accessible globally. & 3 & Users from multiple regions around the world can access the system without performance degradation; average page load under 4 seconds. & 1/10/2025 \\
\end{longtblr}
...
\textbf{FUNC.R.2} & This is an optimization of FUNC.R.1, simply providing improved accessibility for users who are less technical or may not be comfortable with
...
\textbf{FUNC.R.3} & This is an additional optimization of FUNC.R.1 which provides further accessibility improvements for users who may not know exactly what
...
\begin{itemize}
    \item \textbf{APP.R.1:} The system shall resemble a webstore or online boutique in which possibly useful queries are packaged and presented for selection to guide 
    a non-technical user audience.
    \item \textbf{APP.R.2:} The system shall not overwhelm the user with too many or overly complex visible features as to not intimidate a non-technical user audience.
    \item \textbf{APP.R.3:} The system shall present the filtering and searching capabilities of the system in a way that is familiar and intuitive, such as the 
...
\begin{itemize}
    \item \textbf{EOU.R.1:} The system shall have a very low ease of use floor. This floor being scrolling through
    pre-packaged query options and clicking on the one most relevant to their needs.
    \item \textbf{EOU.R.2:} The system shall have a reasonably low ease of use ceiling with intuitive an intuitive filtering interface
...
\begin{itemize}
    \item \textbf{LEA.R.1:} Assuming that the user is familiar with nature of the data itself, the system should
    ...
    \item \textbf{LEA.R.2:} The system shall provide options to generate available metrics or visualizations based on the queried
...
\begin{itemize}
    \item \textbf{UAP.R.1:} The system shall hide the technical details of its queries from the users and only present naturally understandable information
    about the result set (i.e. show applied filters, natural language summary of the results).
    \item \textbf{UAP.R.2:} The general interface of the system should only include words that are naturally understandable to a non-technical audience (i.e. Add filters vs.
...
\begin{itemize}
    \item \textbf{SAL.R.1:} The system shall return all query results within 2 seconds for result sets of up to 5,000 records.
    \item \textbf{SAL.R.2:} All network requests shall have latency below 100 ms under normal operating conditions.
\end{itemize}
...
\begin{itemize}
    \item \textbf{POA.R.1:} All query results shall be accurate and filtered according to user specifications.
    \item \textbf{POA.R.2:} There shall be no inconsistencies between the source database and query results.
\end{itemize}

\subsection{Robustness or Fault-Tolerance Requirements}
\begin{itemize}
    \item \textbf{ROFT.R.1:} The system shall log and report all input errors without crashing.
    \item \textbf{ROFT.R.2:} The backend shall handle errors gracefully, storing detailed logs for debugging and monitoring purposes.
\end{itemize}

\subsection{Capacity Requirements}
\begin{itemize}
    \item \textbf{CAP.R.1:} The system shall support up to 250 concurrent users and handle 5,000 transactions per hour.
    \item \textbf{CAP.R.2:} The database and associated controllers shall be able to manage up to 11 TB of data.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{SOE.R.2:} Under heavy user loads, the system shall maintain at least 80\% of its performance efficiency.
\end{itemize}

\subsection{Longevity Requirements}
\begin{itemize}
    \item \textbf{LON.R.1:} The system shall be designed to operate reliably for at least 5 years, with multiple teams and developers able to maintain and extend it.
    \item \textbf{LON.R.2:} The system shall be compatible with any operating system when running offline.
\end{itemize}
...
\begin{itemize}
    \item \textbf{EPE.R.1:} The system shall be able to run on a standard desktop with atleast 12 GB RAM and average processor.
    \item \textbf{EPE.R.2:} The software shall function correctly on Windows, macOS, and Linux operating systems.
    \item \textbf{EPE.R.3:} The software should be able to run in normal office environment conditions which includes temperatures between 15°C and 30°C. 
\end{itemize}

\subsection{Wider Environment Requirements}

\begin{itemize}
    \item \textbf{WE.R.1:} The system shall be able to run the top three popular browers.
\end{itemize}
...
\begin{itemize}
    \item \textbf{IWAS.R.1:} Data exchanged with adjacent systems shall use JSON format and be transmitted over HTTPS.
\end{itemize}

\subsection{Productization Requirements}
\begin{itemize}
    \item \textbf{PROD.R.1:} The system will use Docker and Kubernetes as it's envoirment for assebility to run on all systems.
    \item \textbf{PROD.R.2:} The system will provide all code changes and software updates on the GitHub Repository. 
\end{itemize}

\subsection{Release Requirements}
\begin{itemize}
    \item \textbf{REL.R.1:} The system releases shall follow the versioning defined by MAJOR\#.MINOR\#.PATCH\#.
    \item \textbf{REL.R.2} The system will provide all code changes and software updates on the GitHub Repository. Code then can be pulled and tagged from the appraite GitHub release.
\end{itemize}
...
\begin{itemize}
    \item \textbf{MAI.R.1:} Due to the open source nature of this system, maintenance shall be possible for those who are not the original developers.
    The future design documentation shall provide the necessary information to accomplish this
    \item \textbf{MAI.R.2:} The system shall be able to immediately accomodate the addition of new datasets into the \gls{frdr}
...
\begin{itemize}
    \item \textbf{SUP.R.1:} The system shall be released alongside a user manual to support users in helping them understand
    the capabilities of the system and the methods for producing useful requests to the system.
    \item \textbf{SUP.R.1:} The system shall be self-supporting outside of the user manual as the developers will not be able
...
\begin{itemize}
    \item \textbf{ADA.R.1:} The system is expected to run on Windows 10 and later, Linux and MacOS.
    \item \textbf{ADA.R.1:} The system is expected to run on the top three most popular web browsers.
    \item \textbf{ADA.R.1:} The system is expected to maintain reasonable performance for users using devices
...
\par{\textbf{INT.R.1:} The product shall prevent itself, the databases, and other files within it from intentional abuse.}
...
\begin{itemize}
    \item \textbf{IMM.R.1:} The system shall be immune to unauthorized attemps to alter data queries, inject code or exterior data, or overwhelm 
the system with an excessive ammount of requests. 
    \item \textbf{IMM.R.2:} All inputs must be validated with network traffic being served exclusively over HTTPS, 
...
\par{\textbf{LEG.R.1:} The system must uphold all software licenses of third-party and integrated systems, as well as adhere to the MIT license defined in the project repository. \newline \indent We have consent from our 
...
\begin{itemize}
    \item \textbf{STA.R.1:} The platform should respect the licensing and citation requirements of \gls{frdr}. 
    \item \textbf{STA.R.2:} The system must uphold all guidelines pertaining to ethical use of animal research data.
\end{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Functional Requirements Included:} FUNC.R.1 (filter data), FUNC.R.2 (NLP query support)
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Functional Requirements Included:} FUNC.R.3 (categorized browsing), FUNC.R.4 (behavioral metrics), FUNC.R.5 (visuals)
    \item \textbf{Non-Functional Requirements Included:}
    \begin{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Functional Requirements Included:} FUNC.R.6 (download data), FUNC.R.7 (global accessibility)
    \item \textbf{Non-Functional Requirements Included:}
    \begin{itemize}
